"""
Update all Kipawa references to Plotra
...
async def update_database():
    ...
    async with async_session() as session:
        try:
            # Update users with 'plotra_admin' role (from old 'kipawa_admin' value)
            result = await session.execute(text("""
                UPDATE users 
                SET role = 'plotra_admin' 
                WHERE role = 'kipawa_admin'
            """))
            print(f"Updated {result.rowcount} users from 'kipawa_admin' to 'plotra_admin'")
...
def update_enum_definitions():
    ...
    if os.path.exists(user_model_path):
        ...
        new_content = content.replace("KIPAWA_ADMIN = \"plotra_admin\"", "PLOTRA_ADMIN = \"plotra_admin\"")
    ...
    if os.path.exists(auth_path):
        ...
        new_content = content.replace("UserRole.KIPAWA_ADMIN", "UserRole.PLOTRA_ADMIN")
        new_content = new_content.replace("require_plotra_admin = require_role([UserRole.KIPAWA_ADMIN])", 
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 50)
    print("Done! All Kipawa references updated to Plotra.")
    print("=" * 50)