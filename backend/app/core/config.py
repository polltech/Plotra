...
logger.info(f"Database name env: {os.environ.get('KIPAWA_DATABASE__NAME', 'not set')}")
...
class SMSConfig(BaseModel):
    ...
    api_key: str = ""
    api_secret: str = ""
    username: str = "SWIFTCOINS"
    sender_id: str = "PLOTRA"
...
class Settings(BaseSettings):
    ...
    def from_yaml(cls, config_path: Optional[str] = None) -> "Settings":
        ...
        if os.path.exists(config_path):
            ...
            # Preprocess: interpolate environment variable placeholders
            for section, values in config_data.items():
                if isinstance(values, dict):
                    for key, value in values.items():
                        if isinstance(value, str) and value.startswith('${') and value.endswith('}'):
                            var_part = value[2:-1]
                            var_name, default = var_part.split(':-')
                            env_value = os.environ.get(var_name, default)
                            # Try to convert to proper type (bool, int, etc.)
