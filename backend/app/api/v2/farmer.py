...
async def create_farm(
    ...
    Includes EUDR compliance fields.
    """
    # Import enums
    from app.models.farm import FarmStatus, PlantingMethod, IrrigationType, SoilType, SlopeCategory, CanopyCover, MonitoringFrequency, OwnershipType
    
    # Always create a new farm (farmers can have multiple farms)
    farm = Farm(
    ...
    if farm_data.ownership_type:
        try:
            farm.ownership_type = OwnershipType(farm_data.ownership_type)
        except:
            pass
    ...
    # Create parcels with EUDR fields
    for parcel_data in farm_data.parcels:
        ...
        # Add EUDR fields to parcel
        if farm_data.land_registration_number:
            parcel.land_registration_number = farm_data.land_registration_number
        if farm_data.altitude_meters:
            parcel.altitude_meters = farm_data.altitude_meters
        if farm_data.soil_type:
            try:
                parcel.soil_type = SoilType(farm_data.soil_type)
            except:
                pass
        if farm_data.terrain_slope:
            try:
                parcel.terrain_slope = SlopeCategory(farm_data.terrain_slope)
            except:
                pass
        if farm_data.year_coffee_first_planted:
            parcel.year_coffee_first_planted = farm_data.year_coffee_first_planted
        if farm_data.estimated_coffee_plants:
            parcel.estimated_coffee_plants = farm_data.estimated_coffee_plants
        if farm_data.farm_status:
            try:
                parcel.farm_status = FarmStatus(farm_data.farm_status)
            except:
                pass
        if farm_data.planting_method:
            try:
                parcel.planting_method = PlantingMethod(farm_data.planting_method)
            except:
                pass
        if farm_data.irrigation_type:
            try:
                parcel.irrigation_type = IrrigationType(farm_data.irrigation_type)
            except:
                pass
        
        # EUDR Mixed Farming Declaration
        parcel.practice_mixed_farming = 1 if farm_data.practice_mixed_farming else 0
        parcel.other_crops = farm_data.other_crops
        parcel.livestock_on_parcel = 1 if farm_data.has_livestock else 0
        parcel.livestock_type = farm_data.livestock_types
        parcel.coffee_percentage = farm_data.coffee_area_percentage or 100
        parcel.crop_rotation_practiced = 1 if farm_data.crop_rotation_practiced else 0
        
        # EUDR Tree Cover & Deforestation
        parcel.trees_planted_last_5_years = 1 if farm_data.trees_planted_last_5_years else 0
        parcel.tree_species_planted = farm_data.tree_species_planted
        parcel.trees_planted_count = farm_data.number_of_trees_planted
        parcel.reason_for_planting = farm_data.tree_planting_reason
        parcel.trees_cleared_last_5_years = 1 if farm_data.trees_cleared_last_5_years else 0
        parcel.reason_for_clearing = [farm_data.reason_for_clearing] if farm_data.reason_for_clearing else None
        if farm_data.current_tree_canopy_cover:
            try:
                parcel.canopy_cover = CanopyCover(farm_data.current_tree_canopy_cover)
            except:
                pass
        
        # Satellite Consent
        parcel.consent_satellite_monitoring = 1 if farm_data.satellite_consent else 0
        parcel.consent_historical_imagery = 1 if farm_data.historical_imagery_consent else 0
        if farm_data.monitoring_frequency:
            try:
                parcel.monitoring_frequency = MonitoringFrequency(farm_data.monitoring_frequency)
            except:
                pass
        
        # Certifications
        parcel.certifications = farm_data.certifications
        if farm_data.certificate_expiry_date:
            parcel.certificate_expiry_date = farm_data.certificate_expiry_date
        parcel.previously_flagged = 1 if farm_data.previously_flagged else 0
        parcel.cooperative_registration_number = farm_data.cooperative_registration_number
        
        db.add(parcel)

    await db.commit()
    result = await db.execute(
        select(Farm).where(Farm.id == farm.id).options(selectinload(Farm.parcels))
    )
...
@router.post("/certifications/upload", status_code=status.HTTP_201_CREATED)
async def upload_certification_file(
    ...
    """
    Upload certification document (Fairtrade, Organic, UTZ, Rainforest Alliance).
    ...
    unique_filename = f"{farm_id}_{certification_type}_{timestamp}{file_ext}"
...
async def get_farmer_stats(
    ...
    """
    # Get farmer's farm (for compliance info)
