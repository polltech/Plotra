...
class PlotraAPI {
    ...
    async request(endpoint, options = {}, retryCount = 0) {
        ...
        // Show loading indicator for non-quick requests
        if (window.showLoader && !options.quiet) {
            window.showLoader();
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // Skip refresh for login/refresh endpoints themselves to avoid loops
                const isAuthEndpoint = endpoint === '/auth/token' || endpoint === '/auth/refresh';
...
Attempted URL: ${url}`;
    ...
    async getDeliveries(filters = {}) {
        ...
        const isAdmin = role === 'plotra_admin' || role === 'super_admin' || role === 'platform_admin' || role === 'admin';
        const isCoop = role === 'cooperative_officer';
        
        let endpoint = '/farmer/deliveries';
        if (isAdmin) {
            endpoint = '/admin/deliveries';
        } else if (isCoop) {
            endpoint = '/coop/deliveries';
        }
    ...
    async getPendingVerifications(filters = {}) {
        console.log('getPendingVerifications called with filters:', filters);
        const params = new URLSearchParams(filters);
        return this.request(`/admin/verification/pending?${params}`);
    }
    
    async approveVerification(recordId, data) {
        return this.request(`/admin/verification/${recordId}/approve`, {
            method: 'POST',
            body: JSON.stringify(data)
        });
    }
    
    async rejectVerification(recordId, reason) {
        return this.request(`/admin/verification/${recordId}/reject`, {
            method: 'POST',
